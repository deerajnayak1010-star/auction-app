...
export class UI {
  ...
  renderLiveMatch(matchState, matchList = []) {
    ...
    // ── NEW BATSMAN ──
    if (s.phase === 'new-batsman' || s.phase === 'new-batsman-then-bowler') {
    ...
    // Non-striker card with manual selection dropdown
    const nonStrikerCard = (b) => {
      if(!b) return '';
      const availNS = s.availableNonStrikers || [];
      const nsDropdown = availNS.length > 0 ? `<select class="lm-ns-select" id="lm-ns-manual-select"><option value="">${b.name}</option>${availNS.filter(p => p.name !== b.name).map(p => `<option value="${p.name}">${p.name}</option>`).join('')}</select>` : '';
      return `<div class="lm-batsman-card"><div class="lm-batsman-name">${b.name}${nsDropdown ? ` <span class="lm-ns-change-wrap">${nsDropdown}</span>` : ''}</div><div class="lm-batsman-stats"><span class="lm-stat-runs">${b.runs}</span><span class="lm-stat-detail">(${b.balls}b)</span><span class="lm-stat-detail">${b.fours}×4</span><span class="lm-stat-detail">${b.sixes}×6</span><span class="lm-stat-sr">SR ${b.balls>0?((b.runs/b.balls)*100).toFixed(1):'0.0'}</span></div></div>`;
    };
    ...
    this.mainEl.innerHTML = `<div class="live-match-page live-match-board-view">${this._renderLiveScoreboard(s, ballDisp)}<div class="scoring-controls"><button class="score-btn" data-ball="0">•</button><button class="score-btn" data-ball="1">1</button><button class="score-btn" data-ball="2">2</button><button class="score-btn" data-ball="3">3</button><button class="score-btn btn-4" data-ball="4">4</button><button class="score-btn btn-6" data-ball="6">6</button><button class="score-btn btn-w" id="lm-wicket-btn" data-ball="W">W</button><div class="score-btn-group"><button class="score-btn btn-extra" id="lm-wd-toggle">WD▼</button><div class="lm-submenu" id="lm-wd-menu" style="display:none;"><button class="lm-sub-btn" data-ball="WD">WD</button><button class="lm-sub-btn" data-ball="WD+1">WD+1</button><button class="lm-sub-btn" data-ball="WD+2">WD+2</button><button class="lm-sub-btn" data-ball="WD+3">WD+3</button><button class="lm-sub-btn" data-ball="WD+4">WD+4</button><button class="lm-sub-btn" data-ball="WD+5">WD+5</button><button class="lm-sub-btn" data-ball="WD+6">WD+6</button><button class="lm-sub-btn" data-ball="WD+ST">WD+St</button><button class="lm-sub-btn" data-ball="WD+RO">WD+RO</button></div></div><div class="score-btn-group"><button class="score-btn btn-extra" id="lm-nb-toggle">NB▼</button><div class="lm-submenu" id="lm-nb-menu" style="display:none;"><button class="lm-sub-btn" data-ball="NB+0">NB+0</button><button class="lm-sub-btn" data-ball="NB+1">NB+1</button><button class="lm-sub-btn" data-ball="NB+2">NB+2</button><button class="lm-sub-btn" data-ball="NB+3">NB+3</button><button class="lm-sub-btn" data-ball="NB+4">NB+4</button><button class="lm-sub-btn" data-ball="NB+5">NB+5</button><button class="lm-sub-btn" data-ball="NB+6">NB+6</button><button class="lm-sub-btn" data-ball="NB+1B">NB+1B</button><button class="lm-sub-btn" data-ball="NB+2B">NB+2B</button><button class="lm-sub-btn" data-ball="NB+3B">NB+3B</button><button class="lm-sub-btn" data-ball="NB+4B">NB+4B</button><button class="lm-sub-btn" data-ball="NB+5B">NB+5B</button><button class="lm-sub-btn" data-ball="NB+RO">NB+RO</button></div></div><div class="score-btn-group"><button class="score-btn btn-extra" id="lm-bye-toggle">BYE▼</button><div class="lm-submenu" id="lm-bye-menu" style="display:none;"><button class="lm-sub-btn" data-ball="B1">1B</button><button class="lm-sub-btn" data-ball="B2">2B</button><button class="lm-sub-btn" data-ball="B3">3B</button><button class="lm-sub-btn" data-ball="B4">4B</button><button class="lm-sub-btn" data-ball="B5">5B</button><button class="lm-sub-btn" data-ball="B6">6B</button><button class="lm-sub-btn" data-ball="B+RO">B+RO</button></div></div><div class="score-btn-group"><button class="score-btn btn-extra" id="lm-lb-toggle">LB▼</button><div class="lm-submenu" id="lm-lb-menu" style="display:none;"><button class="lm-sub-btn" data-ball="LB1">1LB</button><button class="lm-sub-btn" data-ball="LB2">2LB</button><button class="lm-sub-btn" data-ball="LB3">3LB</button><button class="lm-sub-btn" data-ball="LB4">4LB</button><button class="lm-sub-btn" data-ball="LB5">5LB</button><button class="lm-sub-btn" data-ball="LB6">6LB</button><button class="lm-sub-btn" data-ball="LB+RO">LB+RO</button></div></div><button class="score-btn btn-undo" id="live-undo-btn" ${s.canUndo?'':'disabled'}>↩</button><div class="lm-swap-strike-wrapper"><div class="lm-strike-changed-toast" id="strike-changed-toast">⚡ Strike Changed</div><button class="score-btn lm-swap-strike-premium" id="lm-swap-strike-btn"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="17 1 21 5 17 9"></polyline><path d="M3 11V9a4 4 0 0 1 4-4h14"></path><polyline points="7 23 3 19 7 15"></polyline><path d="M21 13v2a4 4 0 0 1-4 4H3"></path></svg><span>Swap</span></button></div></div><div class="lm-batsmen-row">${batsmanCard(striker)}${nonStrikerCard(nonStriker)}</div>${bowlerCard}<div class="live-info-grid"><div class="partnership-card"><div class="partnership-label">Partnership</div><div class="partnership-value">${s.partnership.runs}</div><div class="partnership-balls">(${s.partnership.balls} balls)</div></div><div class="partnership-card"><div class="partnership-label">Extras</div><div class="partnership-value">${ext.total}</div><div class="partnership-balls">W${ext.wides} NB${ext.noBalls} B${ext.byes} LB${ext.legByes}</div></div></div>${mh.length>0?`<div style="margin-top:16px;"><h3 style="font-size:0.9rem;color:var(--text-3);margin-bottom:8px;text-align:center;">📊 Runs per Over</h3><div class="manhattan-chart">${mh.map(o=>`<div class="manhattan-bar" style="height:${Math.max(8,(o.runs/maxMH)*100)}%;background:${s.currentInnings===1?s.teamA.color:s.teamB.color};" data-label="Over ${o.over}: ${o.runs}r"></div>`).join('')}</div></div>`:''}<div style="text-align:center;margin-top:20px;"><button class="btn btn-ghost btn-sm" id="live-back-btn">← Back</button><button class="btn btn-ghost btn-sm" id="live-save-scorecard-btn" style="margin-left:8px;">💾 Save to Scorecard</button></div></div>`;
  }

  _renderLiveScoreboard(s, ballDisp) {
    const overHist = s.overHistory || [];
    const overHistoryHTML = overHist.length > 0 ? `<div class="lm-over-history">${overHist.map(o => `<div class="lm-over-row"><span class="lm-over-num">Over ${o.over}${o.bowlerName ? ': ' + o.bowlerName : ''}</span><span class="lm-over-balls">${o.balls.map(b => ballDisp(b)).join('')}</span><span class="lm-over-total">${o.runs}r</span></div>`).join('')}</div>` : '';
    // Toss info line
